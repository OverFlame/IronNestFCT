...
internal sealed class TaskDispatcher
{
    ...
    private readonly List<ArtilleryTask> _taskQueue = new();
    ...
    public void EnqueueTask(ArtilleryTask task)
    {
        ...
        _taskQueue.Add(task);
        SortQueueByDeadlineAndPriority();
        MelonLogger.Msg($"[FCS Dispatch] queued T{task.targetId}; pending={_taskQueue.Count}");
        TryDispatch();
    }

    /// <summary>
    /// 时间敏感优先级抢占队列：按（开火截止时刻，优先级）升序排列 Pending。截止时刻越早、优先级数值越小越靠前。
    /// 无截止时刻的任务排在有截止时刻之后（视为“就绪即打”，不被定时任务抢占）。
    /// </summary>
    private void SortQueueByDeadlineAndPriority()
    {
        _taskQueue.Sort((left, right) =>
        {
            var leftDeadline = left.fireDeadline ?? float.PositiveInfinity;
            var rightDeadline = right.fireDeadline ?? float.PositiveInfinity;
            var deadlineCmp = leftDeadline.CompareTo(rightDeadline);
            if (deadlineCmp != 0)
                return deadlineCmp;
