...
internal sealed class FirePlanner
{
    ...
    public FirePlan CreatePlan(TaskPlanningResult planning, FirePlanCandidate chosen, float plannedAt)
    {
        ...
        {
            FireDeadline = task.fireDeadline
        };
    ...
    /// <summary>外部显式指定装药时优先采用，否则沿用 Max Charge / 最少装药逻辑。</summary>
    private int ResolveCharge(ArtilleryTask task)
    {
        if (task.requestedCharge is >= 1 and <= 6)
            return task.requestedCharge.Value;

        return _fcs.SceneInteractor.maxCharge ? 6 : BallisticCalculator.MinimumCharge(task.distance);
    }
