...
internal sealed class FirePriorityCoordinator
{
    ...
    public FirePlan ComparePair(FirePlan a, FirePlan b)
    {
        ...
        // 时间敏感抢占：有截止时刻的计划优先按截止时刻升序（越早越先）；无截止的让位给有截止的。
        if (a.FireDeadline.HasValue || b.FireDeadline.HasValue)
        {
            var aDeadline = a.FireDeadline ?? float.PositiveInfinity;
            var bDeadline = b.FireDeadline ?? float.PositiveInfinity;

            if (Mathf.Abs(aDeadline - bDeadline) <= 0.05f)
            {
                first = a.PlannedAt <= b.PlannedAt ? a : b;
                second = ReferenceEquals(first, a) ? b : a;
                reason = "deadline tie; keeping planning order";
            }
            else
            {
                first = aDeadline < bDeadline ? a : b;
                second = ReferenceEquals(first, a) ? b : a;
                reason = $"deadline {aDeadline:F1} < {bDeadline:F1}";
            }
        }
